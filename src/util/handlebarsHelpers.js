...
var register = function (Handlebars) {
    var helpers = {
        parseAnnouncement: function (announcementObj) {
            var announcement = announcementObj.announcement;
            var announcements = announcement.split(/(?:\r\n|\r|\n)/g);
            var link = '<a class="link-on-background" href="' + announcementObj.link + '">' + announcementObj.linkText + '</a>';
            var html = '';

            for (var i = 0; i < announcements.length; i++) {
                var a = announcements[i];

                html += '<p>' + a.replace(announcementObj.linkText, link) + '</p>';
            }
        ...
        isActiveTabClass: function (routerStr, tabStr) {
            return routerStr == tabStr ? 'mdc-tab--active' : '';
        },
